...
class PathFollower(Node):
    def __init__(self):
        ...
        # 🔄 新增：獲取機器人命名空間參數
        self.declare_parameter('robot_namespace', 'B01')  # 預設為B01
        ...
        # 🔄 修改：使用命名空間的Topic名稱
        # Publishers (加上命名空間前綴)
        self.publisher_visual_path = self.create_publisher(
            Path, 
            f'/{self.robot_namespace}/visual_path', 
            10
        )
        
        self.publisher_cmd_vel = self.create_publisher(
            Twist, 
            f'/{self.robot_namespace}/cmd_vel',  # 原來是 '/cmd_vel'
            10
        )
        
        self.publisher_markers = self.create_publisher(
            MarkerArray, 
            f'/{self.robot_namespace}/segment_markers',  # 原來是 '/segment_markers'
            10
        )
        
        self.publisher_current_goal = self.create_publisher(
            Marker, 
            f'/{self.robot_namespace}/current_goal_marker',  # 原來是 '/current_goal_marker'
            10
        )
        
        self.publisher_apf_forces = self.create_publisher(
            Marker, 
            f'/{self.robot_namespace}/apf_forces',  # 原來是 '/apf_forces'
            10
        )
        
        # 🔄 修改：使用命名空間的Topic名稱
        # Subscribers (加上命名空間前綴)
        self.subscription_path = self.create_subscription(
            Float32MultiArray,
            f'/{self.robot_namespace}/path',  # 原來是 '/path'
            self.path_callback,
            10
        )
        ...
        self.subscription_odom = self.create_subscription(
            Odometry,
            f'/{self.robot_namespace}/odom',  # 原來是 '/odom'
            self.odom_callback,
            10
        )
        ...
        self.subscription_scan = self.create_subscription(
            LaserScan,
            f'/{self.robot_namespace}/scan',  # 原來是 '/scan'
            self.scan_callback,
            qos_profile_sensor_data
        )
        
        self.get_logger().info(f"🤖 Path follower for {self.robot_namespace} has been started")
